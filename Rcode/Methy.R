...
### MCF to find DMR
library(DSS)
ALL <- read.table("/Users/xiaoyudai/Documents/multiple-testing/Comparison/CD4_ES/ALL.txt", header=T)
### under FDR nominal level 0.1 ##############
MCF = ALL[,14]
Qvalue = ALL[,15]
...
dmrs2 <- callDMR(data2, p.threshold=0.01,minlen=50,dis.merge=100,pct.sig=0.5)
dim(dmrs2)
...
data1[,4] = (ALL[,8]+ALL[,10])/(ALL[,9]+ALL[,11])
data1[,5] = data1[,3]-data1[,4]
dmrs1 <- callDMR(data1, p.threshold=0.01,minlen=500,dis.merge=100,pct.sig=0.7)
dim(dmrs1)
...
data2[,4] = (ALL[,8]+ALL[,10])/(ALL[,9]+ALL[,11])
data2[,5] = data2[,3]-data2[,4]
dmrs2 <- callDMR(data2, p.threshold=0.01,minlen=500,dis.merge=100,pct.sig=0.7)
dim(dmrs2)
